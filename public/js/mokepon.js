...
let modoJuego='pvp',misionActual=null,progresoAventura=null
...
function renderGuardianes(){
  const contenedor=$('contenedorTarjetas');contenedor.innerHTML='';$('selector-skins').hidden=true;mokepones.forEach((m)=>{contenedor.insertAdjacentHTML('beforeend',`<input type="radio" name="mascota" id="${m.nombre}" value="${m.nombre}"><label class="tarjeta-de-mokepon" for="${m.nombre}"><img src="${m.foto}" alt="Guardián ${m.nombre}"><span class="card-copy"><span class="card-element">${m.elemento}</span><span class="card-name">${m.nombre}</span><span class="card-lore">${m.leyenda}</span></span></label>`)});document.querySelectorAll('input[name="mascota"]').forEach((input)=>input.addEventListener('change',()=>renderSkins(input.value)))
}
...
function prepararMapaLocal(){
  const nombreEnemigo=misionActual?.enemy||mokepones.filter((m)=>m.nombre!==mascotaJugador)[aleatorio(0,1)].nombre,base=mokepones.find((m)=>m.nombre===nombreEnemigo)
